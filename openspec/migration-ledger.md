...
| `fix-company-alias-conflicts` | 經使用者接受後 archive。0052 partial unique index（`alias_lookup_key` 在 confirmed 範圍唯一），TTI Macao 歸還 UN240278、創科補入 UN240278（集團 distinct 37→42；逐公司相加 39→44，差 2 為共同申請去重）。已同步主要規格 CMP-010～012 | 2026-08-18 |
| `add-company-entity-table` | 經使用者接受後 archive。0053 建 `derived_layer.companies`（80 筆回填、`is_temp` 衍生欄），`company_group_members` 外鍵 `ON UPDATE CASCADE`／`ON DELETE RESTRICT`。⚠ 行為變更：刪除仍在集團中的代碼改為回 409。範圍取「丙」，別稱表那條外鍵未做。已同步 CMP-013～014、DAT-008～009 | 2026-08-18 |
| `queue-normalization-candidates` | 經使用者接受後 archive。0054 建 `company_normalization_asked` 蓋章表；候選改排隊（`last_asked_at NULLS FIRST`）＋「有新專利才重問」；一次 20 個切 4 段各 5 呼叫、逐段隔離、失敗段不蓋章；剩餘數與失敗段上畫面。修正舊決策「契約錯誤整批拒絕」為「拒絕該段」。順帶修好中文名線兩處 by-code DELETE 跨線清掉正規化待審建議。已同步 CMP-015～021 | 2026-08-18 |
