...
8. **解讀以整份報告為脈絡**：匯出的 HTML 是一份要給人看的報告，不是卡片堆疊。
   共同背景（涵蓋範圍、母體組成、排除項）要進得了撰寫端，各章節判讀不得互相牴觸。
   ⚠ 敘述統計本身不產生解讀——它是確定性事實，性質同第 5 點的固定文案。
...
| 11 | **匯出 HTML 就是一種報告**：章節之間要有論證連貫，不是卡片堆疊 | 使用者 2026-08-21 |
| 12 | 敘述統計**不單獨產生解讀**，但其他章節寫作時要知道它 | 使用者 2026-08-21 |
| 13 | 敘述統計在前端預覽**單獨一個區塊**（已於 `restructure-html-report` 切片 D-a 實作） | 使用者 2026-08-21 |
...
| 8c | 共同背景（涵蓋範圍、母體組成、排除項）確實進得了撰寫端 | 檢視派工內容，不是「檔案裡有」就算 |
| 8d | 各章節引用的範圍與件數**落在共同背景之內** | 逐章比對年份區間與件數 |
| 8e | 敘述統計**沒有**被產生解讀條目 | 檢視 `narratives.json` 的 report_key 清單 |
| 8f | 🔴 **整份讀一遍**：章節之間可銜接，後段不重複交代同一組背景 | 通讀匯出檔全篇，這條只能人工看 |
