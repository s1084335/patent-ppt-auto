...
## 四之三、匯出 HTML 是一份報告，不是卡片堆疊（EXP-027）

使用者 2026-08-21：「HTML 檔案匯出是要給人家看的……報告呈現和其他報表章節邏輯要串起來，
**等於 HTML 檔案就是其中一種報告了**」。

這句話改變了解讀的定位。現況是**逐卡孤立寫**：

```
prompt 第 3 點：「讀取 report_data.json：sections 鍵列出全部卡片…
                對每張卡片的每個變體…每一變體產一組三件套」
```

⇒ 撰寫端拿到的視野是「這一張卡」，不是「這份報告」。後果是每章各自從零交代背景，
而且沒有機制保證各章的年份區間與件數彼此一致。

### 共同背景就在檔案裡，只是沒被指引使用

`report_data.json` 頂層已經有：

| 鍵 | 內容 | prompt 有提嗎 |
|---|---|---|
| `descriptive_stats` | 件數／族數／受理局／年份範圍／類型／狀態分布 | ❌ |
| `cover_stats` | 件／族／受理局／類型三分法 | ❌ |
| `patent_kind` | 三分法與設計案排除說明 | ❌ |
| `population` | 每張報表的「母體 X/Y 件（原因）」 | ❌（部分經 `section["note"]` 間接傳到） |

⚠ **「檔案裡有」不等於「用得到」**——這正是 `#452` 的形狀：它讀過 `data_access.md`
才決定不取證。驗收判準 8c 因此寫「檢視派工內容，不是『檔案裡有』就算」。

### 敘述統計不產生自己的解讀

使用者：「不一定要單獨解讀」。這與 EXP-014 同一條理由——它是確定性事實，
讓 AI 每次用不同措辭複述只會讓讀者以為口徑變了。

⇒ 要的是**其他章節寫作時知道它**，不是替它寫一段。

### ⚠ 這條最容易走歪的地方

「章節要串起來」很容易被實作成「規定每章開頭先寫一句承接前章」——那就是
**第一世代模板化的完全複製**（見 design 四 · 規則 1）。

EXP-027 的四個 scenario 刻意全部寫成**性質**：背景進不進得了撰寫端、
引用的數字落不落在範圍內、有沒有替敘述統計寫解讀、後段有沒有重複交代同一組背景。
🔴 沒有任何一條規定句型或段落結構。
...
| EXP-027 共同背景進撰寫端 | `test_narrative_prompt.py`（新增：斷言派工內容含共同背景） |
| EXP-027 不與背景牴觸／可銜接 | 🔴 通讀匯出檔全篇，人工；程式化只能驗年份區間與件數落在範圍內 |
| EXP-027 敘述統計不產生解讀 | 斷言 `narratives.json` 的 report_key 不含敘述統計 |
...
| 🔴 「章節要串起來」被實作成「每章開頭先寫一句承接前章」 | **高**。是最直觀的做法，也是第一世代模板化的完全複製 | design 四之三末段；EXP-027 四個 scenario 全部只驗性質、不驗結構 |
| 共同背景只是塞進檔案卻沒被指引使用 | 中。`#452` 已示範過「檔案裡有 ≠ 用得到」 | 驗收判準 8c 驗派工內容本身 |
