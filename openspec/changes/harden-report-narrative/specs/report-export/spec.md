...
### Requirement: EXP-027 解讀 SHALL 以整份報告為脈絡

匯出的 HTML 報表是一份交付給讀者的**報告**，不是卡片的堆疊。撰寫解讀時，
系統 SHALL 把整份報告的共同背景——資料涵蓋範圍、母體組成、排除項——提供給
撰寫端，使各章節的判讀彼此一致且可銜接。

各章節的解讀 SHALL NOT 與該共同背景牴觸（例如引用超出涵蓋範圍的年份，或把已
排除的件數算進來）。

⚠ 敘述統計 SHALL NOT 產生自己的解讀——它是確定性事實，性質同 EXP-014。
本需求要求的是**其他章節寫作時知道它**，不是替它寫一段。

#### Scenario: 共同背景進入撰寫端

- **WHEN** 系統派工產製解讀
- **THEN** 資料涵蓋範圍與母體組成 SHALL 隨派工提供給撰寫端
- **AND** SHALL NOT 只存在於報表產物中而未被指引使用

#### Scenario: 解讀不與共同背景牴觸

- **GIVEN** 報告的資料涵蓋範圍與母體組成已確定
- **WHEN** 檢視各章節的解讀
- **THEN** 各章節引用的範圍與件數 SHALL 落在該背景之內

#### Scenario: 敘述統計不產生解讀

- **WHEN** 解讀產出
- **THEN** SHALL NOT 為敘述統計產生解讀條目
- **AND** 敘述統計的文字 SHALL 維持由確定性程式產生

#### Scenario: 章節之間可銜接

- **WHEN** 讀者依序閱讀整份報告
- **THEN** 後續章節的判讀 SHALL 可承接前面章節已建立的事實
- **AND** SHALL NOT 每一章都從零重新交代同一組背景
