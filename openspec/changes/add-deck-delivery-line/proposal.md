...
1. 流程與閘門不動、**組版輸出層改造（B 案）**：
   ...
   截圖（目視內建）→ **窄 SVG→DrawingML 轉換器**產原生 PPTX
   （逐行文字定位寫死、關 wrap——PowerPoint 零重排自由）」。
   🔴 **本項於 2026-08-13 當日撤銷後又恢復**（同一題裁決三次）：完整沿革與
   第 3 次的三項實測依據見 design 4-0。要點——CLI 目視能力已實測、B 案在最難
   目視的「行首標點」反而更強（從目視項變引擎保證項）、且 B 案的 PowerPoint
   需求只在**開發期映射校驗**，產線不需 Office 授權／互動 session／DCOM。
...
原 Q1「視覺驗收定位」的裁決史——**同一題裁決三次**：

1. 2026-08-12：裁為 B 案。目視走 Chromium 截 SVG（＝截成品，PPTX 已無重排自由）；
   PowerPoint COM 降為開發期一次性映射校驗，不進產線。
2. 2026-08-13 上午：**撤銷**。Windows-only 定案後「零 Windows 依賴」失去意義，
   使用者定「用現在的 deck 流程繼續接進系統就好」；目視改走 COM，PowerPoint
   成為部署前置。
3. 🔴 2026-08-13 下午：**恢復 B 案**（現行）。使用者確認後定案。三項實測依據：
   CLI 目視能力已實讀 baseline 驗證；B 案在最難目視的「行首標點」更強
   （斷行寫死 → 從目視項變引擎保證項）；B 案的 PowerPoint 只用在**開發期**
   映射校驗，產線不需 Office 授權／互動 session／DCOM——4-0b 的三個部署條件
   因此去掉兩個。

⚠ 之所以完整保留這段來回：下次有人問「為什麼翻兩次」，要看得到每次的依據，
才不會第四次憑印象翻回去。

曾評估之替代案（Windows 轉圖代理／LibreOffice／Aspose）的取捨留 design.md §4
備查。LibreOffice 已由 `pptx_to_png.py` 檔頭實測否決（中文字型／斷行不可信）。
