...
- [x] ~~2.2c 圖表字達 14pt（依 SVG 高度反推圖區幾何）~~
      ...
      **實測數據轉為追溯**（2026-08-13，14 張真實 SVG，跑 fit 候選 26–16）：
      | 項目 | 結果 |
      |---|---|
      | 達 14pt | **1／14** |
      | 低於 9pt 硬門檻 | 2／14（機會矩陣，1120×610）|
      | 實際能放大到 | 16–22px，**沒有一張到 26px**（撞版擋住）|
      | 三條路的天花板 | 圖區高 +0.3pt／字級被撞版擋／降 SVG 高＝減資料列 |
      ⚠ 撞版門檻與圖高無關：扁圖（156px）只能到 16、高圖（556px）反而能到 22
      ——因為標籤欄寬與 chip 背景寬是用原字級算死的。
      ⚠ chip 重排實測：機會矩陣 610px→534px、字級 15.1→20，投影片 7.8pt→11.9pt
      （跨過 9pt 門檻但仍不到 14）。原生繪製後這個重排也不再是必要手段。
