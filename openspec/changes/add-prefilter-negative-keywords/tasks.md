...
## 切片 C-2：AI 對命中專利建議留或剔（PRE-008，2026-08-21 補立案）

🔴 **這一步原本漏了**。規格七條需求裡 AI 只出現在「轉英文比對詞」（PRE-002），
沒有「讀命中專利給建議」——而使用者的原始需求就寫著「CLI 根據負面關鍵字，
去篩給建議，然後人工確認保留或剔除」。2026-08-21 補 `PRE-008`。

⚠ 我一度自作主張做成「命中集 vs 母體的 IPC 分布對照」，那不是使用者要的
...
- [x] E.9 把 `ai:keyword_expand` 與 `ai:prefilter_review` 的刷新目標從 `[]`
...
⚠ 2026-08-21 範圍擴大：討論後多了**範圍描述輸入**、**命中原因顯示文本**、
**AI 建議欄**與 **SSE 增量填入**。原本四段變六段。

- [x] E.1 Red：瀏覽頁有入口且顯示待辦數（WSP-013）
- [x] E.2 Red：瀏覽頁不承載關鍵字編輯／確認／裁決操作（WSP-013）
- [x] E.3 Green：初階篩選頁六段（範圍描述／關鍵字／確認比對詞／命中預覽／
      待裁決／剔除名單）
- [x] E.4 Green：`renderMain()` 加分派、**不加左導覽項**（沿案件比對前例）
- [x] E.5 Green：瀏覽頁入口，待辦數取自權威 API 不由前端自數
- [x] E.6 ⚠ 確認畫面不得顯示 SQL——補測試斷言頁面不含 SQL 片語
- [x] E.7 Green：命中原因顯示**命中的那段原文**，不只是關鍵字
      （使用者 2026-08-21：「命中原因改顯示文本」）
- [x] E.8 Green：AI 建議欄。🔴 四種狀態各有明確樣子，
      **尚無建議與無判讀依據不得留白**——空白會被讀成「沒問題」
- [x] E.9 Green：把兩條 AI 線的刷新目標從 `[]` 改成實際資源
- [x] E.10 Green：SSE 建議**增量填入**（使用者 2026-08-21：
      「陸續填上，但不要整頁重載也不要影響到使用者」）
      - 🔴 只換建議格，不重繪表格、不動勾選、不重排、不重套篩選
      - 🔴 建議格**預留固定高度**：填入撐高列會把使用者正在看的位置往下推，
        那正是「影響到使用者」
      - ⚠ 已裁決的列不因建議到達而回到待裁決（後端已守，前端不得自己補畫）
- [x] E.11 驗收：實機開頁走完一輪（範圍描述 → 輸入 → 轉換 → 確認 → 套用 →
      AI 建議 → 裁決 → 還原）
...
#### 🔴 但第一版這條寫錯了（使用者提問當場照出來）

使用者問「目前系統同筆專利支援不同 workspace?」→ 實查正式庫：
**281 筆專利每一筆都同時屬於全庫與專屬 workspace**（資料模型無任何約束，
使用者補充「三個以上喔不是兩個」）。

而我的第一版把**全庫也算成一個 workspace** ⇒ `purge_candidates` **恆為空**，
任何專利都永遠刪不掉。

⚠ 這個 bug 從測試與 dry-run 都看不出來：正式庫回 0 筆，我當成
「因為還沒有剔除紀錄」——**兩個原因都成立，症狀完全一樣**。
（事後驗證：把保留期設 0 天仍是 0 筆，證明這次的 0 是真的沒有剔除紀錄。）

修正：`membership` CTE 加 `WHERE w.is_global IS NOT TRUE`。全庫是總覽、
依定義收錄所有專利，它的成員身分不是使用者的範圍決策而是自動的。
⚠ 排除的只是「擋刪除」；刪掉後全庫的成員名單**仍要清**，否則留孤兒 id。

補三條測試：全庫不擋刪除／三個 workspace 全部釋出才准刪／刪後全庫名單也清乾淨。
