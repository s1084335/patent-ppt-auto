...
## 4. 內容專業度：與範例的落差（2026-08-10 實機驗收 r3 後定）

使用者原話：「範例的專業到哪，我們就要到哪」。實機看過 11 頁成品後逐項比對
`report-professionalism-spec.md` 的 B 系列定案與兩份範例，落差如下。

### 4.1 🔴 版型 renderer 沒照範例實作（兩處，使用者指名「很醜」）

| 版型 | 實作現況 | 範例／定案 |
|---|---|---|
| `walls_gaps` | **單框純文字**，4 行字後下半頁全空 | 滑雪機 p3 是**兩欄**：「⚠ 需迴避的專利牆（高密度）」vs「建議切入的空白區」，每欄多個項目卡 |
| `stat_callout` | 大數字取到 **88**（assignments 總數，**無意義**）＋主題列表 | 定案 B-4 該頁是**表格 4 欄**，根本不該用 stat_callout |

⚠ `stat_callout` 是**掉圖時的降級目的地**；p7 出現它是因為
`cluster_topic_table:opportunity_effect` 反查不到圖。要一併處理降級路徑。

### 4.2 🔴 主題類沒照 B-3／B-4 的表格規格

| 定案 | 現況 |
|---|---|
| **B-3 技術主題**：表格 5 欄「技術主題｜件數｜參與公司｜主要玩家｜代表專利」，代表專利＝前三大申請人各取一件的**專利號**；敘述**點名代表專利講內容**（「馬達自鎖阻力機構：帝瑪斯 CN307437423——法蘭擋板＋電磁鐵解鎖，2024 年 5 件同架構」） | 寫成 `walls_gaps` 純文字，無表格、無專利號 |
| **B-4 功效主題**：表格 4 欄「功效主題｜件數｜主要技術手段｜代表專利」，`主要技術手段` 由 AI 讀該主題**前五筆**專利的 `文獻備註` 統整 | 降級成 `stat_callout` 數字卡 |
| **B-2 申請趨勢**：敘述要**點名主題標籤**（「2024：馬達自鎖阻力機構 5 件、虛擬互動 1 件」）——即「技術類要和時間套著看」 | 只寫件數與家族數，沒點主題 |

### 4.3 🔴 章節缺五塊

| 缺的 | 範例出處 | 影響 |
|---|---|---|
| **研發切入建議** | 滑雪機 p3／割草機 p2 | 最大目標「供開發部決定切入方向」**無人回答** |
| **KP 深入 ×3 頁**（前 10 大申請人） | 滑雪機 p8–10 | 只有一張象限圖；使用者要求「後面多家申請人抓出來講」 |
| **機會評估綜合頁** | 割草機 p6 | 專利密度×競爭者結構強度四象限＋逐象限行動 |
| **三條結語** | 滑雪機 p11 右區 | 判讀說明頁只有偏差、沒有結語 |
| **附錄主題全表** | 割草機 p8 | 正文引用回查的依據 |

### 4.4 🔴 技術內容深度不足——沒下到專利層

使用者原話：「技術內容更細的資訊沒去找」。現況寫到**主題名＋佔比**
（「馬達自鎖6件帝瑪斯獨占5件（83%）」），範例寫到**機構層**
（「法蘭擋板＋電磁鐵解鎖」「同一線軸反向捲繞：操作繩 vs 彈性回繩」）。

⚠ 取證入口早就備好（`patent_ids` 已進 `applicant_strength_profile`、
`文獻備註` 可查、`query_database` 可用），但規劃 prompt 沒要求下到這一層。

### 4.5 根因與修法方向

**規劃 prompt 只給了「版型庫 18 種、依內容決定出哪幾頁」，沒有把 B 系列的頁面
規格告訴 CLI**。`content_standard.md` 有敘事鏈的表，但沒有 B-3／B-4 的欄位規格、
沒有「哪些頁必出」、沒有「技術內容要下到機構層」的具體要求。

修法分兩層：
1. **規則層**：把 B 系列規格寫進 `content_standard.md`（skill 唯一標準），
   規劃端與組版端都讀得到。
2. **實作層**：修 `walls_gaps` 兩欄 renderer、修 `stat_callout` 取數字邏輯、
   補 `opportunity_quadrant` 的圖檔反查（掉圖根因）。

## 5. r5 實機逐頁驗收（2026-08-10，12 頁全數轉圖檢視）

檔案：`report_trial_20260810_124138.pptx`。使用者逐頁指出＋Claude 逐頁比對範例。

### 5.1 使用者直接指出（3 項）

| # | 問題 | 頁 |
|---|---|---|
| 1 | **不要兩張圖合在一起講** | p4／p5／p8 皆 `comparison` |
| 2 | **研發切入建議與結論頁只留一個**——都是報告總結 | p2 `exec_summary` ＋ p11 `direction` |
| 3 | **p2 排版要改**：現在是兩欄字牆、六條長文字並排，「誰看得懂呢」 | p2 |

### 5.2 Claude 逐頁比對出的（10 項）

| # | 問題 | 頁 | 嚴重度 |
|---|---|---|---|
| 4 | **主題表格只印 1 列**（`顯示前 1/13 筆`），CLI 只好把整張表塞進要點文字 | p6 | 🔴 |
| 5 | **要點溢出頁尾**，與母體註記重疊看不清 | p6／p7 | 🔴 |
| 6 | **內部欄名印在頁面上**：`jurisdiction_distribution`／`applicant_country_matrix`／`applicant_ranking`／`applicant_strength_profile` | p4／p8／p9 | 🔴 違反 content_standard 四之三 |
| 7 | **表格欄位錯**：實際 7 欄（含「技術狀態」「前三大占比」），定案 B-3 是 5 欄、B-4 是 4 欄；且使用者已定「技術狀態先不用」 | p6／p7 | 🔴 |
| 8 | **標題含規格說明**：「（5 欄表格）」「（4 欄表格）」「以 CPC 圖佐證覆蓋不足」 | p6／p7／p12 | ⚠ |
| 9 | p12 標題溢成兩行，且判讀頁掛了 CPC 圖（本該純敘述） | p12 | ⚠ |
| 10 | p11 上排三卡大量留白（態勢／機會／方向各只有 2–3 行，卡片高度固定） | p11 | ⚠ |
| 11 | p8 右圖列英文人名（Grayson Hugh Bourne…），與左圖中文公司名對不起來 | p8 | ⚠ |
| 12 | p5 左圖只有 2 根長條（A63B 42／F03G 2），半頁空白 | p5 | ⚠ |
| 13 | `points_dropped: 10`——10 條要點被組版丟棄 | 全域 | 🔴 |

### 5.3 🔴 KP 分頁完全沒照 B-6 定案

| B-6 定案 | r5 實際 |
|---|---|
| **3 頁**（KP①②③） | **1 頁** `kp_cards` |
| **依有無技術軌跡分組**（軌跡判準＝不同申請年 ≥3 個） | 混在一起，未依軌跡分 |
| KP① 帝瑪斯／曾晴要**展開「共同 12 件」與「各自獨立」** | 只寫「合計 15 件去重」，沒拆 |
| KP③ 六家各一句 | 只提到兩家 |

⚠ 版面帳也對不上：定案算過「IPC/CPC 不出 2 頁 → 空出的位置給 KP①②③」，
但 r5 的 IPC／CPC **出了**（p5），佔掉 KP 的頁數。

⚠ 根因同前：**B-6 的分頁規格沒寫進規劃 prompt**——只寫了「用 kp_* 版型把多家
抓出來講」，沒給軌跡判準、三頁配置、共同申請要拆開。

### 5.4 使用者新增的兩條規則（2026-08-10）

1. **KP 深入不要專利號**——版面有限，要留給更有價值的內容。
2. 🔴 **朝濃縮前進，但濃縮不是留白**——「不是把版面空出來，而是每個區塊寫的
   都要是最有價值的內容」。

   例（p10）：
   ```
   現在：拉繩滑雪(219208876 轉軸繞線套)→風磁複合(EP4104909 風量盤導柱撥磁座)
   濃縮：拉繩滑雪 轉軸繞線套 → 風磁複合 風量盤導柱撥磁座 → 馬達自鎖 法蘭擋板電磁解鎖
   ```
   同樣版面，拿掉專利號後多寫得下一個機構——那才是濃縮。

### 5.6 🔴 r6 頁序改版定案（2026-08-10 使用者裁決）

反覆看過兩份範例後逐題裁決的結果。原則：**綜合＝取兩者之長後合併，不是頁數相加**
（使用者原話「重複的不要單純加上去」）。

| 決策 | 內容 |
|---|---|
| 結論頁 | **只留 `direction`**，三個具名發現寫進「依據」欄；取消 `exec_summary` |
| 技術主題 | 正文用**主題演進圖**（早期 vs 近期），不用表格 |
| 功效主題 | 正文用**機會四象限板**；使用者：「機會四象限是好的，所以至少功效主題要用這個呈現」 |
| 表格 | 降到**附錄**，保留代表專利號供回查（「先正文用圖、表格進附錄，有需要會再修」） |
| 專利號 | **正文任何一頁都不放**，尤其 KP 深入 |
| 申請人排名 | **留**——象限圖看定位、看不到數量；要看單獨／共同／已轉讓三段組成 |
| 年度矩陣 | **留**——唯一能看出各家進場時序與連續性的圖；解讀可精簡 |
| IPC/CPC 門檻 | 驗收**暫時停用**（`PPT_CLASSIFICATION_MIN_L4=0`）以確認版面本身正確；**實機部署要生效**。「4 階沒出現，5 階就不會有」＝兩階同屬一個 report_key，是結構保證 |
| 有圖／無圖頁 | **兩種寫法**，不是同一種寫法配不同字數（使用者：「不然就又會悲劇」） |
| skill | 規則要同步進 skill 且**保持通用**——範例是品質標杆，不是內容模板 |

**落實位置**（三處必須一致，否則規則等於沒有）：
`skills/patent-report-ppt/content_standard.md`（skill 唯一來源）、
`skills/patent-report-ppt/SKILL.md`（版型庫與寫作分流摘要）、
`backend/app/worker/report_planning_runner.py`（系統實際跑的 prompt，`PROMPT_VERSION` bump 為 v2）。

**門檻缺陷修復**（本輪一併修）：門檻原本只擋 `_expand_page_layout`（組版自排頁）路徑，
SlidePlan 路徑整條繞過去，低於門檻的 IPC 照樣進了 r5 的 p5。
⚠ manifest 還記了 `below_threshold_skipped: 2`，反而讓人以為擋掉了——
**判定算對、警告發了、沒有人據以行動**，是這類缺陷的共同特徵。
修為三道：`ppt_eligible_variant_keys`（CLI 拿不到圖）→ `drop_below_threshold_pages`
（plan 路徑）→ `_report_key_has_data`（固定頁路徑），三者共用同一個判準。

### 5.5 做得好、不要改壞的部分

- **p9 KP 象限圖**：泡泡分布清楚、定位分類有圖例、要點具名到「祺驊只押捲輪回捲一類主題」
- **p10 機構層描述**：「轉軸繞線套→風量盤導柱撥磁座→法蘭擋板電磁解鎖」
  ——取證權限的價值就在這裡
- **p11 建議頁**：方向｜依據｜行動三欄結構正確，底部核心結論條有收斂
- **p12 判讀頁**：四條偏差＋三條結語齊全，「曾晴為帝瑪斯共同申請人、兩者 12 件
  重疊，排名須合併理解」這種提醒到位
