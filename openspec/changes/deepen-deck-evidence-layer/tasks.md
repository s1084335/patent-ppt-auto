...
- [ ] 3.7 🔴 **設計專利在結論頁沒有任何落點——待使用者裁決**（2026-08-19 提出）
  ...
  🔴 **2026-08-19 使用者裁決：獨立一頁「設計保護結論」**（未動工）

  - 用 §7c 的通用 `layout: "table"` 版型另開一頁，不併進主題結論頁。
  - 欄位走「申請人｜策略型態｜判讀｜專利行動」；`策略型態` 逐字取自
    `design_protection_strategy` 的 `strategy_type`（引擎已產，比照發現欄
    的逐字閘門），`判讀`／`專利行動` 由 CLI 寫、動詞仍限 `ACTION_VERBS`。
  - ⚠ **不併進主題結論頁的理由**：設計保護是 per-applicant 維度，母體也不同
    （全庫 11 件 vs 分群 44）。塞進同一張表會讓兩個母體混在一起，讀者看到
    同一張表會預設同一個母體——那正是本輪 §1 在防的病。
  - ⚠ 這頁的行動動詞表沿用同一份 `ACTION_VERBS`，**不另開設計專用動詞**；
    要擴充走 openspec 留痕，不在 prompt 放寬。
