...
現行是「表 → 圖 → 解讀」，把唯一的分析產出排在最底。改為解讀緊接標題。

⚠ **解讀目前掛在每個 chart-panel 內（逐變體）**，不是章節層級——多變體章節
（IPC L4／L5）各變體有各自的解讀。改版後：**解讀仍逐變體**，但整塊移到圖之前，
隨切換鈕一起換。單變體章節就是一段文字接在標題下。

無解讀時維持既有 pending 標示（不留空白、不假裝有內容）。
...
現行 `<details>` 展開機制沿用，只改**預設顯示列數 20 → 5**，並把 summary 文字
改為「數據表（共 N 列，預設顯示前 5 列）」。合計列（`totals-cell`）行為不變。
...
- **HTML 結構契約**（純函式層，不需瀏覽器）：`render_index` 產出的 HTML 內
  導覽項數＝章節數、每章 DOM 順序為 解讀→圖→表、`id="sec-*"` 齊備、
  表格預設列數 5、`h1` 非寫死英文。
