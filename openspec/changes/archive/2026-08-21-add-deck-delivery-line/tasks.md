...
- [x] 2.1b **圖表原生繪製——前半完成，後半停用**（2026-08-14 使用者裁決）
      🔴 **2026-08-14 追加裁決：轉換器封存、詞彙邊界獨立接上。**
      - 封存＝自工作樹移除（`svg_to_pptx.py`＋兩個測試檔），git 歷史保存。
        **復活指標：`git show 422eec0:skills/html-report-to-deck/scripts/svg_to_pptx.py`**
        （該版含完整狀態說明與接上觸發條件；測試檔同 commit 挖回）。
      - 有在發揮功能的部分＝**詞彙邊界**，獨立為 `svg_canvas.SVG_VOCABULARY`
        （產出端唯一定義處）＋`test_svg_vocabulary_guard`——實組**全部頁型**
        （八頁型＋結論頁＋六型圖形，14 頁）逐頁掃元素 ⊆ {svg, rect, text, image}。
        原本由轉換器測試間接守的詞彙鎖由它直接承接。
      - 接上觸發條件不變：目視迴圈實跑出「SVG 過但 pptx 錯」的實例。
