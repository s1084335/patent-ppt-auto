...
THRESHOLD_BASIS: dict[str, BasisDeclaration] = {
    ...
    "STATUS_RECENT_WINDOW_YEARS": BasisDeclaration(
        ...
        "🔴 2026-08-20 兩批實測：滑雪機切出早期 9 年（資料起自 2011）、"
        "割草機早期 6 年（資料起自 2014），**兩批都切得出可用的早期窗**。"
        "⚠ 但這只證明「在這兩批不會壞」，沒證明 5 是對的數字——"
        "一批資料若只跨 2022–2026，早期窗會空掉（`derive_status_windows` 會回 None）。",
        derivation="仍待justify：需要能說明「近期」為何是 5 年的理由，"
                   "而非只是與既有行為相容",
        pending=True),
    ...
    "STATUS_TAIL_PENDING_RATIO": BasisDeclaration(
        ...
        reference="兩批實測夾出：50% 會多排除滑雪機的 2024（未決 50%）、"
                  ...
                  "現行窗。🔴 2026-08-20 割草機分群後復核：兩批都排除 2 年，"
                  "窗分別為 (2011,2019)/(2020,2024) 與 (2014,2019)/(2020,2024)，"
                  "與各自的資料起點相符。⚠ 待第三批資料再檢驗此區間是否仍成立",
        pending=True),
    ...
    "STATUS_STAGNANT_BAND": BasisDeclaration(
        ...
        "\n"
        "🔴 2026-08-20 **兩批實測，先前的擔憂被縮小了**（割草機分群後取得第二批）："
        "滑雪機窗長 9 vs 5（持平主題 ratio 0.357）、割草機 6 vs 5（0.455）。"
        "改成年均後帶內外翻轉：滑雪機 **1/5**、割草機 **0/10**，"
        "而最終狀態變動 **0 個**（成長規則的優先序排在帶之前，多數主題走不到）。"
        "⚠ 結論：窗長汙染只在窗高度不對稱時才咬得到，而推導基準已吸收大部分。"
        "改年均是**正確但目前無效益**的改動——它會動現有數字卻改不了任何判定。"
        "仍標 pending 是因為帶寬未被 justify，不是因為機制急待重寫。",
        derivation="待重新設計：要嘛正規化成年均，要嘛改用不受窗長影響的統計量"
                   "（兩批實測顯示不急，等出現窗長極度不對稱的批次再處理）",
        pending=True),
    "STATUS_MIN_SAMPLE": BasisDeclaration(
        ...
        "🔴 2026-08-20 兩批驗證（割草機分群後）：滑雪機擋掉 0/5（最小主題 6 件）、"
        "割草機擋掉 2/10（被擋的兩個各 3 件）。擋掉 3 件的主題正是本門檻要做的事"
        "——3 件切成兩窗後任何比例都是噪音。門檻行為與設計意圖相符。"
        "⚠ 這是**驗證**不是**依據**：依據在 `reference`（Cochran），"
        "實測只能說明「它沒壞」，不能反過來當成它為何取 5 的理由。",
