...
| 可靠度下限 | 「資料少到這個計算沒有意義」的下限 | 要附**統計慣例或可從程式內部推導**的依據 |

## ⚠ 「可靠度下限」為什麼**不該**改成推導（2026-08-19 加）

使用者原則「不能是絕對值」對**比較型**判準完全成立——「比整體快」的基準隨批次變，
寫死就是拿甲批的尺量乙批。但**可靠度型**判準問的是另一件事：
「資料夠不夠讓這個計算有意義」。

⚠ 把它改成推導是**循環論證**：「這批資料少，所以把門檻調低」——
但資料少正是計算不可靠的時候。3 件就是 3 件，不會因為這批總量大就變可靠。

故這一類保留絕對值，**但依據必須是統計或演算法的，不得是「本案剛好幾件」**。
後者才是原本的病：用本批的分布去 justify 一個本該與批次無關的門檻。
...
THRESHOLD_BASIS: dict[str, BasisDeclaration] = {
    ...
    "STATUS_TAIL_EXCLUDED_YEARS": BasisDeclaration(
        ...
        derivation="🔴 2026-08-19 使用者裁決：改由 `derive_tail_exclusion` 依"
                   "**法律狀態**推導（未決比 ≥ 門檻且連續的末端年份才排除）；"
                   "本常數降為狀態資料缺席時的退路",
        reference="PCT Article 21(2)(a)／各局早期公開制度：自最早優先日起 18 個月公開",
        pending=False),
    "STATUS_TAIL_PENDING_RATIO": BasisDeclaration(
        "可靠度下限",
        "判定「這一年還沒跑完」的未決比門檻。未決比高代表該年可見案件幾乎都還"
        "沒有結局——看到的只是最早公開的那一小撮，其餘還沒進資料。"
        "⚠ 用狀態而非年份，**不需要知道今天是哪一年**：一份舊資料的末端年份"
        "未決比自然偏高，會被正確排除，報表也不隨時間變動（可重現性保住）。"
        "⚠ 誠實界線：0.60 的判準是「與既有行為相容」，而既有行為（固定排除末 2 年）"
        "**本身沒被獨立驗證過**。它是兩批夾出的相容區間，不是被證明的最佳值。",
        reference="兩批實測夾出：50% 會多排除滑雪機的 2024（未決 50%）、"
                  "70% 會少排除割草機的 2025（未決 67%），只有 60% 兩批都重現"
                  "現行窗。⚠ 待第三批資料再檢驗此區間是否仍成立",
        pending=True),
    ...
    "STATUS_MIN_SAMPLE": BasisDeclaration(
        "可靠度下限",
        "🔴 2026-08-19 依據改寫。原註解是「**本案** 13 個主題有 3 個落在這裡」"
        "——那是用本批的分布去 justify 一個本該與批次無關的門檻。"
        "本門檻問的是「件數夠不夠讓趨勢判斷有意義」：判定會把主題切成早期／近期"
        "兩窗再比計數與比例，而**格內樣本太少時比例不可解讀**"
        "（「近期 2 件 vs 早期 1 件」在數學上是成長 100%，實際是噪音）。"
        "⚠ 誠實界線：程式**沒有真的跑統計檢定**，只是拿總件數當閘門"
        "——引用慣例是為了說明**量級**從何而來，不是宣稱做了檢定。"
        "而且它擋的是主題**總件數**，比「每格 ≥5」寬鬆。",
        reference="Cochran 期望次數規則：列聯表／比例比較的每格期望次數 ≥ 5，"
                  "低於此常態近似不成立。與分群層 `model.small_topic_ratio` 的 "
                  "`min_topic_docs=5` 同量級（⚠ 兩者語意不同——一個是「算不算好的群」、"
                  "一個是「判不判得出趨勢」——故不合併為同一常數）"),
    "MIN_CLUSTERING_DOCUMENTS": BasisDeclaration(
        "可靠度下限",
        "🔴 2026-08-19 依據改寫。原註解是「實機動因：**滑雪機** 60 筆專利」"
        "——那是觸發調整的**場合**，不是門檻該取 30 的**理由**。"
        "真正的理由可以從程式內部推導：`clustering/runner.py` 的 k 掃描規則規定"
        "30–49 筆掃 `k=(3, 5, 8)`，而分群品質指標 `small_topic_ratio` 以 "
        "`min_topic_docs=5` 認定「太小的群」。最小 k=3 × 每群至少 5 篇 ⇒ "
        "**15 筆是絕對下限**；取 30 讓最粗的 k=3 每群約 10 篇，是下限的 2 倍。"
        "⚠ 這一項**不該**改成推導：它決定「這批能不能分群」，"
        "由本批推導等於「資料少就把標準放寬」，而資料少正是分群不可靠的時候。",
        reference="程式內部可推導：`runner.k_candidates` 的 (3,5,8) × "
                  "`model.small_topic_ratio(min_topic_docs=5)` ⇒ 下限 15，取 2 倍"),
