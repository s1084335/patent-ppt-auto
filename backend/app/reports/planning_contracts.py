...
# 需要 12 行但該版型只有 11 行，最後一條被整條丟棄。prompt 給的是品質指引；
# 這裡只擋明顯離譜的值——若卡在 31 字就讓整個 job 失敗，使用者會什麼都看不到，
# 那比少一條要點更糟。
#
# 🔴 2026-08-10 上調 5 → 8，且**對齊組版端實測容量**（不是憑感覺訂）：
#
# 實測落差顯示無圖頁（摘要／建議／判讀）內容量只有範例三分之一
# （118 字／5 塊 vs 範例 357 字／25 塊）。改 prompt 要求「無圖頁至少 6 個單元」後，
# CLI 照做寫了 6 條卻被舊上限 5 打回，整份規劃失敗（實測 job 278）
# ——⚠ 規則與守門互相矛盾比兩者都寬鬆更糟：使用者什麼都拿不到。
#
