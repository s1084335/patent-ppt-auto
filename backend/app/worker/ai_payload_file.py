...
# 單批資料上限（字元）。資料落檔解決了命令列 32,767 的限制，但沒解決
# **AI 的 context window**——一次塞太多，模型讀不完或推理品質下降。
# 100KB 是保守值：遠低於 context 上限，且讓模型有餘裕逐篇閱讀後歸納。
# 實測參考：118 筆專利／10 主題×5 篇＝128KB，故現行資料量會切成 2 批；
# 5000 筆／40 主題約 512KB → 約 5 批。
