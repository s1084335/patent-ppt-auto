...
3. **100 字是上限、不是目標**：「更少都行，只要精簡清楚就好」。prompt 不得要求寫滿
   或設下限，避免模型灌水湊字數——`build_prompt` 只寫「100 字以內」，不寫任何下限。
...
def build_prompt(batch: Sequence[tuple[int, str]]) -> str:
    ...
    return (
        ...
        f"3. 長度 {NOTE_MAX_CHARS} 字以內（此為上限，更少也可以）。以精簡清楚為準則，\n"
        "   句子夠用就停，不要為湊長度補充空話、不要重複同一件事。\n"
        "4. 只寫獨立項讀得出來的內容，不臆測用途、不加評價、不編造數據。\n"
        "5. 不要輸出「本專利」「本發明」以外的來源標記，也不要標註是誰產生的。\n\n"
        f"{claims_block}\n\n"
