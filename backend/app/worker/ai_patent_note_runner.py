...
3. **字數兩層線＋不截斷**（2026-07-26 定兩層線；2026-07-28 使用者定案移除截斷）：
   prompt 給 **60 字目標線**、**100 字硬上限**，**程式不再硬切**。
   沿革：原本只寫「100 字以內」，模型把上限當目標寫滿，實測幾乎每筆都逾 100 字被
   截斷、斷在句中，故補一條較低的目標線讓模型有餘裕收句。但硬切仍在，使用者實機
   看到殘句後定案「先不要做截斷」——改為**超過即 raise**，該批不寫入、可重跑。
   prompt 明示懲罰機制（超過整批作廢、系統不代為修剪），讓模型自己在 100 字內寫好。
   兩個數字都不是下限——prompt 不得要求寫滿或設下限。
   ⚠ 與主題標籤同口徑（workspace_service 的 label／summary 超過硬上限亦 raise）。
...
#   把目標壓到 60（2026-07-28 由 70 再收緊），模型有餘裕自己把最後一句收完整。
# - NOTE_MAX_CHARS：**硬上限**。2026-07-28 起**不截斷**——超過即 raise，該批不寫入、
#   可重跑；prompt 明示懲罰機制讓模型自己守住。與主題標籤同口徑。
NOTE_TARGET_CHARS = 60
NOTE_MAX_CHARS = 100
...
def build_prompt(batch: Sequence[tuple[int, str]]) -> str:
    ...
    return (
        ...
        f"3. 長度以 {NOTE_TARGET_CHARS} 字為目標，{NOTE_MAX_CHARS} 字為**絕對不可超過的硬上限**。\n"
        "   兩個數字都是天花板而非目標——更短更好，句子夠用就停，不要為湊長度補充空話、\n"
        "   不要重複同一件事。\n"
        f"4. ⚠ **懲罰機制**：任何一筆超過 {NOTE_MAX_CHARS} 字，**整批全部作廢**、不寫入資料庫，\n"
        f"   系統會判定你未遵循指示並要求重做。系統**不會**幫你截斷或修剪——寫超過就是失敗。\n"
        f"   因此請在 {NOTE_MAX_CHARS} 字內把話講完整：先想好要講什麼，再動筆，\n"
        "   **最後一句必須完整並以句號結束**。寧可寫短寫完整，絕不可寫長。\n"
...
def _extract_notes(parsed: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if "notes" not in candidate and isinstance(candidate.get("result"), str):
        ...
        try:
            ...
            raise PatentNoteRunnerError(str(exc)) from exc
    ...
    *,
    ...
    for index, batch in enumerate(batches, start=1):
        ...
        for item in _extract_notes(parsed):
            ...
            if not note:
                continue
            # ⚠ 2026-07-28 使用者定案「先不要做截斷」：超過硬上限視為模型未遵循指示，
            # 直接失敗讓該批可重跑——不再對 note 做切片硬切。
            # 硬切會斷在句中變殘句（使用者實機看到後判定「這樣很不好」），
            # 且照寫等於預設「上限只是建議」，模型會逐漸放飛。
            # 與主題標籤同口徑：workspace_service 的 label／summary 超過硬上限亦是
            # raise 讓呼叫端重生，不靜默截斷。
            if len(note) > NOTE_MAX_CHARS:
                raise PatentNoteRunnerError(
                    f"patent_id {patent_id} 的備註 {len(note)} 字，超過硬上限 "
                    f"{NOTE_MAX_CHARS} 字；模型未遵循長度指示，本批不寫入（可重跑）。"
                )
            pairs.append((patent_id, note))
