...
3. **字數兩層線、程式完全不干預**（2026-07-26 定兩層線；2026-07-28 兩次調整定案）：
   prompt 給 **60 字目標線**、**100 字上限**，**程式不截斷也不攔截，原樣寫入**。
   沿革（三階段，都由實機結果驅動）：
   a. 原本只寫「100 字以內」→ 模型把上限當目標寫滿，實測幾乎每筆逾 100 字被
      `note[:100]` 硬切、斷在句中（「…第二段減小，形」）。故補一條較低的目標線。
   b. 使用者看到殘句後定「先不要做截斷」→ 改為超過即 raise 整批作廢。
      但實機 job #67 一筆 103 字（只超 3 字）害**整批 0 筆寫入**，代價遠大於效益。
   c. 使用者定案「**不用有懲罰，他超過也可以寫**」→ 兩層線退為純軟性指引。
   兩個數字都不是下限——prompt 不得要求寫滿或設下限。
   ⚠ 與主題標籤**刻意不同口徑**：label／summary 是顯示元素（籤片、圖例），過長會
   破版故 raise；備註是列表欄位，長一點只是換行，不值得為此丟掉整批。
...
# - NOTE_MAX_CHARS：**上限指引**。2026-07-28 定案：程式**不截斷也不攔截**，
#   超過照樣原樣寫入；只在 prompt 告知，由模型自律。
    ...
    *,
    ...
    for index, batch in enumerate(batches, start=1):
        ...
        for item in _extract_notes(parsed):
            ...
            if not note:
                continue
            # ⚠ 2026-07-28 使用者兩次定案的最終結果：**不截斷、也不攔截，原樣寫入**。
            # 沿革：① 原本 note[:100] 硬切 → 斷在句中變殘句，使用者判定「這樣很不好」
            #      ② 改為超過即 raise → 實機 job #67 一筆 103 字（只超 3 字）害整批
            #         0 筆寫入，懲罰落在整批而非那一筆，代價遠大於效益
            #      ③ 使用者定案「不用有懲罰，他超過也可以寫」
            # 字數兩層線（60/100）仍寫進 prompt 當**軟性指引**，由模型自律；
            # 程式端不再做任何長度加工或攔截。
