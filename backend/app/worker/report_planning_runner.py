...
def build_prompt(brief: dict[str, Any]) -> str:
    ...
    return (
        ...
        + "## 品質標準（兩份參考範例的共同 DNA）\n"
          ...
          "- 每個單元＝「短標籤＋一句內容」，例如：\n"
          "  `機械傳動期 2011–2021｜拉繩、立柱滑輪為主，單點機構改良`\n"
          "  `2022 首爆 15 件 13 族｜真成長，非同族灌水`\n"
          "- 分段組織：技術分期用**時代卡**、發現用**編號卡**、建議用**三欄**、\n"
          "  判讀用**偏差四條＋結語三條**\n"
        ...
        "2. **Key Player 深入**：一張象限圖不夠。用 `kp_deepdive`／`kp_cards`／`kp_compare`\n"
        "   把**多家**申請人抓出來講——領導者、對照組、利基新興各有各的技術路線。\n"
        "3. **判讀說明＋三條結語**：收尾要給判斷，不是只講限制。\n\n"
        "## 🔴 主題類與趨勢頁有固定欄位規格（不得寫成純文字頁或大數字頁）\n"
        "- **技術主題分布**＝表格 5 欄「技術主題｜件數｜參與公司｜主要玩家｜代表專利」，\n"
        "  代表專利放**專利號**；敘述要**點名該專利講技術內容**。\n"
        "- **功效主題分布**＝表格 4 欄「功效主題｜件數｜主要技術手段｜代表專利」，\n"
        "  `主要技術手段` 是你**讀該主題前五筆專利的 `文獻備註`**後統整出來的。\n"
        "- **申請趨勢**的敘述要**點名主題標籤**（「2024：馬達自鎖阻力機構 5 件、虛擬互動 1 件」）\n"
        "  ——只寫件數等於沒把技術與時間扣起來。\n\n"
        "## 🔴 技術內容必須下到機構層——這是給你查證權限的理由\n"
        "| ❌ 聚合數字就寫得出來（不合格） | ✅ 讀過專利內容才寫得出來 |\n"
        "|---|---|\n"
        "| 馬達自鎖 6 件，帝瑪斯獨占 83% | 馬達自鎖：**法蘭擋板＋電磁鐵解鎖**，帝瑪斯 5 件同架構 |\n"
        "| 捲輪回捲 10 件 top3 占 80% | 同一線軸**反向捲繞操作繩與彈性回繩**，省掉獨立回位機構 |\n\n"
