...
def build_payload(scope_description: str,
    ...
    return {
        "instruction": (
            "判斷每一筆專利與「這批專利的技術範圍」是否相關，"
            "對每一筆給出建議：keep（屬於這批的範圍，建議保留）或 "
            "exclude（不屬於這批的範圍，建議剔除）。\n"
            "判讀依據只能是該筆的標題、摘要與獨立項內容，"
            "以及本檔提供的這批專利範圍描述。不得依賴其他知識或臆測。\n"
            "理由用繁體中文，一句話說明它與這批專利範圍的關係，"
            "並指出你是從標題、摘要還是獨立項看出來的。\n"
            "🔴 理由是**寫給使用者看的**，不要提到本資料檔的任何欄位名稱或"
            "英文鍵名（例如不要寫「title 為…」「屬於 batch_scope 的…」），"
            "請用「標題」「摘要」「獨立項」「這批專利的範圍」這類說法。\n"
