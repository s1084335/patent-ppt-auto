...
# 案件比對流程最新定案（2026-07-22）

本段為目前案件比對的可讀最新定案區；下方舊內容先保留，待後續重整時再處理亂碼與歷史版本。

## 核心定位

案件比對是一條獨立任務線，但不是完全外掛在專利系統之外。案件比對匯入的專利資料要寫入資料庫、進入專利總覽、可被追蹤，也要經過既有 preprocessing / embeddings 流程。差別在於這批資料需帶有「案件比對」用途標籤，並放在總覽的主題分類標籤中，讓使用者能區分它不是一般情報分析匯入批次。

案件比對流程與一般報表流程分開：報表線處理 workspace 專利情報分析；案件比對線處理特定被比對來源與比對來源之間的權利要求 / 技術要素比對。兩者可共用專利號機制、資料庫、embedding、job、artifact 與 AI 任務基礎設施。

## 兩種資料角色

### 被比對來源

被比對來源是使用者要拿來分析、理解、拆要素並與其他資料比對的案件資料。來源可以是：

1. 從既有專利庫用專利號機制搜尋選取。
2. 從檔案匯入單筆或多筆專利。

若從檔案匯入，資料要寫入資料庫、進入專利總覽、標籤為「案件比對」，並走 embeddings 流程。多筆匯入可以建立多個案件比對任務；一筆被比對案件對應一個 `case_comparison` 任務。

### 比對來源

比對來源是拿來和被比對來源比對的參考專利資料。來源可以是：

1. 根據 embeddings 查詢現有資料庫，找出語意相關的既有專利。
2. 使用者另外匯入他的專利，寫入資料庫後再次理解與 embedding。

案件比對不限制同時使用既有專利庫資料與檔案匯入資料；限制點是資料角色要清楚，不能混淆哪一批是被比對來源、哪一批是比對來源。

## 使用流程

1. 使用者建立案件比對任務。
2. 使用者指定被比對來源：既有專利庫搜尋或檔案匯入。
3. 若為檔案匯入，系統寫入資料庫、加入專利總覽、標記「案件比對」，並觸發 preprocessing / embeddings。
4. 系統或使用者指定比對來源：透過 embeddings 查詢現有資料庫，或另外匯入專利並重新理解。
5. AI 對被比對來源進行權利要求理解與要素拆解。既有粗糙拆解成果可作為基礎，但正式流程需可人工檢查、可修改、可版本化、可追溯到原文。
6. 使用者核准 AI understanding。未核准前不得進入逐要素比對。
7. 系統進入 `element_analysis`，逐項列出 claim element、對應比對來源 evidence、比對狀態、理由、備註與待補資料。
8. 使用者檢查、修正並保存 element analysis 版本。
9. AI 依正式 element analysis 產生技術比對結論草稿。此處只能做技術比對、風險提示與待確認點，不直接給法律結論。
10. 系統依案件比對模板輸出報告 artifact。

## 報告章節規劃

案件比對報告模板以 `docs/reference/報告範例/REXO2039_Opinion_Summary_20251120.pdf` 的專利內容比對寫作格式為基準，不相關章節不沿用。第一版章節規劃：

1. 案件摘要：比對目的、來源、版本、標的範圍。
2. 被比對來源：來源方式、專利號或匯入批次、資料庫追蹤資訊、案件比對標籤。
3. 比對來源：embedding 查詢結果或使用者匯入專利清單。
4. 權利要求理解：AI 拆解結果、使用者核准版本、重要假設。
5. 要素拆解表：claim element、技術意義、引用文本、人工修正紀錄。
6. 逐要素比對表：每個 element 對應比對來源 evidence、狀態、理由、待補資料。
7. 初步技術比對結論：met / not_met / unclear 等技術判斷摘要，不作法律結論。
8. 風險與待確認事項：需要補件、補圖、補訪談或人工確認的點。
9. 附錄：原始輸入、使用者核准紀錄、版本與 artifact metadata。

## 與現有 API 的整合方向

目前既有 API 已有 `create -> target -> understanding -> approve -> element-analysis` 基礎。接下來要補的是：

- 建立案件比對專用匯入 / 來源選擇流程。
- 檔案匯入後要寫入資料庫、進專利總覽、標記「案件比對」，並觸發 embeddings。
- 比對來源需支援 embeddings 查詢現有專利庫，以及使用者另行匯入專利。
- 前端 GET comparison 後要能回填既有 `element_analysis` 供使用者查看與編輯。
- 後續再接 verdict 與案件比對報告輸出。
