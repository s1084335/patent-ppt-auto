...
🔴 **`reading_guide` 的口徑段落一律照抄，不要自己編**：計數單位、同族合併、
共同申請、分類覆蓋這四段的正式說法在 `report_data.json` 的
`table_display.reader_guide`（每則含 `title` 與 `body`），逐字放進本頁要點即可。

⚠ 為什麼不能自己寫：這四段是引擎實際採用的口徑，與各頁頁尾的母體註記同源。
你憑印象改一個字（例如把「件」寫成「family 數」），簡報就會和頁尾註記自相矛盾，
而且沒有任何檢查會擋下來。**可觀測性偏差與資料限制**則仍由你依該 workspace
的實際情況撰寫——那部分沒有固定答案。
