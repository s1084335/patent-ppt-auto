...
| ↑ | ⚠ **依目標規劃時不要用**（2026-08-10 使用者定案）：兩張圖擠一頁各講一半，讀者兩張都看不清楚。一頁一張圖、講透它。保留此版型僅供固定頁序模式與既有報告重產 | |
...
🔴 **有圖頁與無圖頁是兩種寫法**，不是同一種寫法配不同字數——這是最常出事的一條。

- **無圖頁**（`exec_summary`／`walls_gaps`／`reading_guide`／`direction`／
  `kp_deepdive`／`kp_cards`／`kp_compare`）：內容 100% 來自你寫的敘述，
  留空就是一張白框；每頁至少 6 個內容單元。
- **有圖頁**（其餘）：文字只寫**圖上看不出來的**（機構層手段、路線異同、意味什麼），
  3 條、每條約 40 字。複述排名與件數＝整頁只講了一件事。

組版端會掃描正文區並標記 `empty_body`／`text_overflow_estimated`／`points_dropped`
——但它只告警，不會替你補內容或重寫。兩種寫法的完整規格見 `content_standard.md`。
