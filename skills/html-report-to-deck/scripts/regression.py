...
def _content() -> dict:
    ...
    return {
        ...
        # 🔴 2026-08-19（§9.3）：`rec_*`／`recommendations` 已退場，
        # 結論頁是唯一的第 2 頁。⚠ 回歸素材必須跟著換，否則測到的是一條
        # 已經不存在的路徑（而且會 KeyError 在 slide_conclusions 上，
        # 錯誤訊息完全指不到「素材沒跟上」）。
        "conclusions": {
            "title": "綜合結論：各主題的研發意涵與專利行動",
            "takeaway": "本頁測試結論表的三欄寬度與分組標題是否溢出。",
            "covered": "3/4",
            "uncovered": [{"topic": "合成主題 D", "reason": "回歸素材刻意留一項未涵蓋"}],
            "rows": [
                {"topic": f"合成主題 {c}",
                 "finding": "11件/5家｜最大持有 89%｜申請成長",
                 "reading": "依據：11 件 5 家、近期集中度 88.9%，是最擠的一區；"
                            "先讀完那 4 件請求項再落筆。",
                 "evidence": "依據：CN 121754861",
                 "action": act, "pending_count": n}
                for c, act, n in (("A", "細讀比對", 5), ("B", "追蹤", 2),
                                  ("C", ["迴避設計", "追蹤"], 1))],
        },
        "pages": [
