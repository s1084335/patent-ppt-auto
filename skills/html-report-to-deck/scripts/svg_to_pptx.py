...
## ⚠ 現況＝已完成、**未接線**（2026-08-14 使用者裁決「先不接」）

pptx 交付物目前由 `deck_layout.build`（python-pptx 原生組版）產出，**不經過本檔**；
本檔只有測試在跑（`test_svg_to_pptx_converter.py`＋`test_svg_chart_vocabulary.py`，
34 支守著詞彙）。不是死碼——是這條路的**期權**。

不接的理由（實測支撐）：原生路徑與 SVG 的差異只有「長句換行點差一字」
（映射校驗 `output/_verify/mapping/RECORD.md`，內容逐字同、方向安全），
而接上的代價是交付物**編輯性變差**（一行一框、關 wrap，改字不重排）。

🔴 **接上的觸發條件**：目視迴圈實跑出「SVG 過了、pptx 開起來錯」的實例
——一字差異升級成實害時再接（`make_deck` 的 pptx 端改走本檔），
接完必須重跑映射校驗（預期換行差異類消失，剩光柵化粗細）。
